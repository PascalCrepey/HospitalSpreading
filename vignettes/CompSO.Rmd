...
tspan_odin = 0:(tspan_max/time_step) #define a longer number of time steps
...
odin_result <- odin_model$run(0:(tspan_max/time_step))
...
odin_model <- initialize_odin_binomial(beta = beta,
                            n_subpop = npop,
                            size_subpop = starting_pop,
                            I_per_subpop = index_pop,
...
odin_model_binomial100 <- initialize_odin_binomial(beta = beta,
                            n_subpop = npop,
                            size_subpop = starting_pop,
                            I_per_subpop = index_pop,
                            transfer_matrix = trans_mat,
...
odin_model_binomial5 <- initialize_odin_binomial(beta = beta,
                                          n_subpop = npop,
                                          size_subpop = starting_pop,
                                          I_per_subpop = index_pop,
                                          transfer_matrix = trans_mat,
...
odin_binomial100_runs <- odin_model_binomial100$run(0:(tspan_max*100), replicate = Nsims)
odin_binomial5_runs <- odin_model_binomial5$run(0:(tspan_max*5), replicate = Nsims)
odin_poisson_runs <- odin_model_poisson$run(0:tspan_max, replicate = Nsims)
...
, odin_multi %>%
  ...
  summarise(mean = mean(value)
            , lo = max(value)
            , hi = min(value)) %>%
  mutate(package = "Odin binomial step 1")
, odin_binomial5_multi %>%
...
print(benchmark_odin(odin_model_poisson, times = 0:tspan_max, nRun = 10))
print(benchmark_odin(odin_model_binomial5, times = 0:(tspan_max/0.2), nRun = 10))
print(benchmark_odin(odin_model_binomial100, times = 0:(tspan_max/0.01), nRun = 10))
```
