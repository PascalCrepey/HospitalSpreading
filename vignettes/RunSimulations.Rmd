...
odin_result <- run_simulation(odin_model, t_max, time_step)
```
...
## Model output
```{r old_model, echo = F, eval = T}
initialized_model <- initialize_odin_binomial(
  beta = beta/time_step,
  n_subpop = length(size_subpop),
  size_subpop = sum(size_subpop),
  I_per_subpop = I_initial_time,
  transfer_matrix = trans_mat
  )

set.seed(20230317)
odin_result = run_simulation(initialized_model, tspan_max, time_step)
```
